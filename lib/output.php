...
add_action( 'wp_enqueue_scripts', 'genesis_hcc_css' );
/**
 ...
 * @since 2.2.3
 */
function genesis_hcc_css() {

    $handle = defined( 'CHILD_THEME_NAME' ) && CHILD_THEME_NAME ? sanitize_title_with_dashes( CHILD_THEME_NAME ) : 'child-theme';

    $color_link   = get_theme_mod( 'genesis_hcc_link_color', genesis_hcc_customizer_get_default_link_color() );
    $color_accent = get_theme_mod( 'genesis_hcc_accent_color', genesis_hcc_customizer_get_default_accent_color() );
    $logo         = wp_get_attachment_image_src( get_theme_mod( 'custom_logo' ), 'full' );

    if ( $logo ) {
        $logo_height           = absint( $logo[2] );
        $logo_max_width        = get_theme_mod( 'genesis_hcc_logo_width', 350 );
    ...
    $css .= ( genesis_hcc_customizer_get_default_link_color() !== $color_link ) ? sprintf(
        '
    ...
    $css .= ( genesis_hcc_customizer_get_default_accent_color() !== $color_accent ) ? sprintf(
        '
		...
		',
        $color_accent,
        genesis_hcc_color_contrast( $color_accent )
    ) : '';
